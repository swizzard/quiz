CREATE OR REPLACE FUNCTION api.ip_banned(u_ipaddr inet) RETURNS boolean AS $$
  SELECT EXISTS(SELECT 1 FROM api.player p WHERE p.ipaddr = u_ipaddr AND p.banned IS true);
$$ LANGUAGE SQL STABLE;
...
BEGIN
  SELECT api.ip_banned(u_ipaddr) INTO is_banned;
  IF is_banned THEN
    RETURN banned_user;
  ELSE
...
DECLARE u record;
BEGIN
  IF api.ip_banned(u_ipaddr) THEN
    RETURN false;
  ELSE
...
BEGIN
  ...
  FOR key, value IN SELECT * FROM json_each_text(data->'answers') LOOP
    INSERT INTO api.participant_response (participant_id, response, answer_id) VALUES (p_id, value, key::integer);
  END LOOP;
  RETURN true;
END;
$$ LANGUAGE plpgsql VOLATILE;
...
BEGIN
  ...
  IF EXISTS(SELECT 1 FROM api.quiz q WHERE q.id = game_id AND q.creator = user_id) THEN
    INSERT INTO api.game (quiz_id) VALUES (game_id) RETURNING code INTO game_code;
    RETURN game_code;
  ELSE
...
BEGIN
  ...
  round_count := 0;
  UPDATE api.quiz SET name = q_name WHERE id = q_id AND creator = creator_id;
  IF NOT FOUND THEN
    RETURN false;
  ELSE
    FOREACH rnd IN ARRAY ARRAY(SELECT json_array_elements(questions)) LOOP
      ...
      IF rnd_id IS NULL THEN
        INSERT INTO api.quiz_round (quiz_id, round_no) VALUES (q_id, round_count) RETURNING id INTO rnd_id;
      END IF;
      FOREACH q IN ARRAY ARRAY(SELECT json_array_elements(rnd->'questions')) LOOP
        qe_id := (q->>'questionId')::integer;
        IF qe_id IS NULL THEN
          INSERT INTO api.question (round_id, question, question_no) VALUES (rnd_id, q->>'question', q_no) RETURNING id INTO qe_id;
        ELSE
          UPDATE api.question SET question = q->>'question'
            FROM api.quiz_round qr
            JOIN api.quiz ON qr.quiz_id = quiz.id
            WHERE api.question.id = qe_id
            AND api.quiz.creator = creator_id;
        END IF;
        FOREACH a IN ARRAY ARRAY(SELECT json_array_elements(q->'answers')) LOOP
          ...
          IF ans_id IS NULL THEN
            INSERT INTO api.answer (question_id, answer, points) VALUES (qe_id, a->>'answer', (a->>'points')::integer);
          ELSE
            UPDATE api.answer SET answer = a->>'answer', points = (a->>'points')::integer
            FROM api.question qe
            JOIN api.quiz_round qr ON qe.round_id = qr.id
            JOIN api.quiz ON qr.quiz_id = quiz.id
            WHERE api.answer.id = ans_id
            AND api.quiz.creator = creator_id;
          END IF;
        END LOOP;
        q_no := q_no + 1;
